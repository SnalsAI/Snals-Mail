...
class ServizioDetailResponse(ServizioResponse):
    """Servizio con tipi appuntamento associati"""
...
class DisponibilitaBase(BaseModel):
    ...
    durata_slot_minuti: Optional[int] = None  # Override durata default tipo appuntamento
    note: Optional[str] = None
...
class DisponibilitaCreate(DisponibilitaBase):
    staff_id: int
    sede_id: int
    servizio_id: Optional[int] = None  # Servizio di riferimento (obbligatoria per coerenza date)
    tipi_appuntamento_ids: Optional[List[int]] = None  # Se vuoto, tutti i tipi sono permessi
...
class DisponibilitaBulkCreate(BaseModel):
    ...
    staff_id: int
    sede_id: int
    servizio_id: Optional[int] = None  # Servizio di riferimento
    date_list: List[date]
    ora_inizio: time
    ora_fine: time
    durata_slot_minuti: Optional[int] = None  # Override durata default tipo appuntamento
...
class DisponibilitaResponse(DisponibilitaBase):
    ...
    created_at: Optional[datetime] = None
    tipi_appuntamento: Optional[List[TipoAppuntamentoResponse]] = None
    servizio: Optional[ServizioResponse] = None
...
class SlotPrenotazioneInfo(BaseModel):
    """Info minime sulla prenotazione per gli slot del calendario"""
    model_config = ConfigDict(from_attributes=True)
    id: int
    contatto_nome: Optional[str] = None
    contatto_cognome: Optional[str] = None
    contatto_email: Optional[str] = None
    contatto_telefono: Optional[str] = None


class SlotCalendarioResponse(SlotDetailResponse):
    """Slot con info prenotazione per il calendario staff"""
    prenotazione: Optional[SlotPrenotazioneInfo] = None
...
class ContattoBase(BaseModel):
    ...
    provincia: Optional[str] = None
    iscritto_snals: Optional[bool] = None
