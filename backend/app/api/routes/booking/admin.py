...
Gli admin possono:
- Gestire sedi, staff, tipi appuntamento, servizi
...
from app.models.booking import (
    ...
    BookingServizio, BookingServizioTipoAppuntamento, BookingDisponibilita,
...
from app.schemas.booking import (
    ...
    # Servizio
    ServizioCreate, ServizioUpdate, ServizioResponse, ServizioDetailResponse,
    # Slot
...
@router.get("/servizi", response_model=List[ServizioDetailResponse])
async def lista_servizi(
    attivo: Optional[bool] = Query(None),
    auth: bool = Depends(require_admin_auth),
    db: Session = Depends(get_db)
):
    """Lista servizi con tipi appuntamento."""
    query = db.query(BookingServizio)
    if attivo is not None:
        query = query.filter(BookingServizio.attivo == attivo)
    return query.order_by(BookingServizio.data_inizio.desc()).all()


@router.post("/servizi", response_model=ServizioDetailResponse)
async def crea_servizio(
    data: ServizioCreate,
    auth: bool = Depends(require_admin_auth),
    db: Session = Depends(get_db)
):
    """Crea una nuova servizio."""
    ...
    existing = db.query(BookingServizio).filter(BookingServizio.slug == data.slug).first()
    if existing:
        raise HTTPException(status_code=400, detail="Slug già esistente")

    servizio = BookingServizio(
        nome=data.nome,
    ...
    if data.tipi_appuntamento_ids:
        for tipo_id in data.tipi_appuntamento_ids:
            ...
            if tipo:
                servizio.tipi_appuntamento.append(tipo)
...
@router.put("/servizi/{id}", response_model=ServizioDetailResponse)
async def aggiorna_servizio(
    id: int,
    data: ServizioUpdate,
    auth: bool = Depends(require_admin_auth),
    db: Session = Depends(get_db)
):
    """Aggiorna una servizio."""
    servizio = db.query(BookingServizio).get(id)
    if not servizio:
    ...
    if data.tipi_appuntamento_ids is not None:
        servizio.tipi_appuntamento.clear()
        for tipo_id in data.tipi_appuntamento_ids:
            tipo = db.query(BookingTipoAppuntamento).get(tipo_id)
            if tipo:
                servizio.tipi_appuntamento.append(tipo)

    servizio.updated_at = datetime.utcnow()
    db.commit()
    db.refresh(servizio)
    return servizio


@router.delete("/servizi/{id}", response_model=SuccessResponse)
async def elimina_servizio(
    id: int,
    auth: bool = Depends(require_admin_auth),
    db: Session = Depends(get_db)
):
    """Disattiva una servizio."""
    servizio = db.query(BookingServizio).get(id)
    if not servizio:
        raise HTTPException(status_code=404, detail="Servizio non trovata")
    ...
    servizio.updated_at = datetime.utcnow()
    db.commit()
    return SuccessResponse(message="Servizio disattivata")
...
async def lista_prenotazioni(
    ...
    query = db.query(BookingPrenotazione).join(BookingSlot, BookingPrenotazione.slot_id == BookingSlot.id)
    ...
    if servizio_id:
        query = query.filter(BookingPrenotazione.servizio_id == servizio_id)
    if stato:
...
async def esporta_prenotazioni(
    ...
    query = db.query(BookingPrenotazione).join(BookingSlot, BookingPrenotazione.slot_id == BookingSlot.id).join(BookingContatto)
...
async def statistiche(
    ...
    # Prenotazioni oggi
    prenotazioni_oggi = db.query(func.count(BookingPrenotazione.id)).join(BookingSlot, BookingPrenotazione.slot_id == BookingSlot.id).filter(
        func.date(BookingSlot.data_ora_inizio) == oggi
    ).scalar()

    # Prenotazioni settimana
    prenotazioni_settimana = db.query(func.count(BookingPrenotazione.id)).join(BookingSlot, BookingPrenotazione.slot_id == BookingSlot.id).filter(
        func.date(BookingSlot.data_ora_inizio) >= inizio_settimana,
        func.date(BookingSlot.data_ora_inizio) <= oggi
    ).scalar()

    # Prenotazioni mese
    prenotazioni_mese = db.query(func.count(BookingPrenotazione.id)).join(BookingSlot, BookingPrenotazione.slot_id == BookingSlot.id).filter(
    ...
    per_sede = dict(db.query(
        BookingSede.nome,
        func.count(BookingPrenotazione.id)
    ).select_from(BookingPrenotazione).join(
        BookingSlot, BookingPrenotazione.slot_id == BookingSlot.id
    ).join(BookingSede, BookingSlot.sede_id == BookingSede.id).group_by(BookingSede.nome).all())
    ...
    per_tipo = dict(db.query(
        ...
        BookingSlot, BookingPrenotazione.slot_id == BookingSlot.id
    ).join(BookingTipoAppuntamento, BookingSlot.tipo_appuntamento_id == BookingTipoAppuntamento.id).group_by(BookingTipoAppuntamento.nome).all())
