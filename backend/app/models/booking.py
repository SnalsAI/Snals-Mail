...
- BookingCampagna: Campagne/eventi (es. GPS 2025)
...
class BookingTipoAppuntamento(Base):
    ...
    campagne = relationship(
        "BookingCampagna",
        secondary="booking_campagna_tipo_appuntamento",
...
class BookingCampagna(Base):
    ...
    prenotazioni = relationship("BookingPrenotazione", back_populates="campagna")

    def __repr__(self):
        return f"<BookingCampagna {self.id}: {self.nome}>"


# Tabella ponte Campagna <-> TipoAppuntamento (N:M)
class BookingCampagnaTipoAppuntamento(Base):
    """Associazione N:M tra campagne e tipi appuntamento"""
    __tablename__ = "booking_campagna_tipo_appuntamento"

    campagna_id = Column(
        Integer,
        ForeignKey("booking_campagne.id", ondelete="CASCADE"),
        primary_key=True
    )
...
class BookingSlot(Base):
    ...
    prenotazione = relationship("BookingPrenotazione", back_populates="slot", uselist=False)
...
class BookingContatto(Base):
    ...
    scuola_attuale = Column(String(255))  # Nome o codice della scuola dove lavora
    provincia = Column(String(100))
...
class BookingPrenotazione(Base):
    ...
    campagna_id = Column(Integer, ForeignKey("booking_campagne.id"), nullable=True, index=True)
    ...
    campagna = relationship("BookingCampagna", back_populates="prenotazioni")
